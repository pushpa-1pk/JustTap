...
export default function BookingDetailsScreen() {
  ...
  const { data: timelineRes } = useGetCustomerBookingTimelineQuery(bookingId, { skip: !bookingId });
  ...
  const { coordinates: liveCoords } = useTracking(booking?.status === 'ON_THE_WAY' ? booking._id : null);
  ...
  const handlePaymentContinue = async () => {
    ...
    Alert.alert(
      ...
      `Order ${gatewayOrderId} is ready in the backend. Complete the Razorpay client handoff before calling payment verification from the app.`
    );
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {__DEV__ && ['ARRIVED', 'SERVICE_STARTED'].includes(booking.status) && (
          <View style={[styles.devBanner, { backgroundColor: `${colors.primary}15`, borderColor: colors.primary }]}>
            <Text style={[typography.bodySmall, { color: colors.warning, fontWeight: '800' }]}>DEVELOPER DEBUG KEY</Text>
            <Text style={[typography.bodyMedium, { color: colors.text, marginTop: 4 }]}>
              Give this code to the provider to advance the job:
            </Text>
        ...
        <View style={[styles.timelineCard, { backgroundColor: colors.surface, borderColor: colors.border, padding: 16 }]}>
          <View style={styles.billRow}>
            ...
            <Text style={[typography.bodyMedium, { color: colors.text }]}>Rs {booking.priceSnapshot.basePrice}</Text>
          </View>
          <View style={styles.billRow}>
            <Text style={[typography.bodyMedium, { color: colors.textSecondary }]}>Convenience Fee</Text>
            <Text style={[typography.bodyMedium, { color: colors.text }]}>Rs {booking.priceSnapshot.platformFee}</Text>
          </View>
          <View style={styles.billRow}>
            <Text style={[typography.bodyMedium, { color: colors.textSecondary }]}>Taxes & GST</Text>
            <Text style={[typography.bodyMedium, { color: colors.text }]}>Rs {booking.priceSnapshot.taxAmount}</Text>
          </View>
          <View style={[styles.divider, { backgroundColor: colors.border }]} />
          <View style={styles.billRow}>
            <Text style={[typography.bodyLarge, { color: colors.text, fontWeight: '700' }]}>Total Amount</Text>
            <Text style={[typography.h2, { color: colors.secondary, fontWeight: '800' }]}>Rs {booking.priceSnapshot.finalAmount}</Text>
          </View>
        </View>
          ...
          <View style={styles.actionGroup}>
            ...
            >
              {isCreatingOrder ? (
                ...
                <Text style={[typography.buttonText, { color: colors.onPrimary }]}>Pay Now (Rs {booking.priceSnapshot.finalAmount})</Text>
              )}
            </Pressable>
          </View>
        )}
          ...
          <>
            ...
            <View style={[styles.timelineCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              {timeline.map((entry, index) => (
                <View key={`${entry.timestamp}-${index}`} style={styles.timelineEntry}>
        ...
        <View style={styles.modalBg}>
          <View style={[styles.modalCard, { backgroundColor: colors.surface }]}>
            ...
            <TextInput
              ...
              placeholder="Changed my mind / Selected wrong slot"
        ...
        <View style={styles.modalBg}>
          <View style={[styles.modalCard, { backgroundColor: colors.surface, padding: 24 }]}>
            ...
            <View style={styles.paymentSummary}>
              ...
              <Text style={[typography.h1, { color: colors.text, fontWeight: '800' }]}>Rs {payAmount}</Text>
            </View>

            <Pressable style={[styles.payBtnSubmit, { backgroundColor: colors.secondary }]} onPress={handlePaymentContinue}>
              <Text style={[typography.buttonText, { color: '#FFFFFF' }]}>Continue With Gateway</Text>
            </Pressable>

            <Pressable style={[styles.payBtnDismiss, { marginTop: spacing.md }]} onPress={() => setPayModalVisible(false)}>
